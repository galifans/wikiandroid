...
override fun addView(view: View, params: ViewGroup.LayoutParams) {
    applyDefaultToken(params)
    mGlobal.addView(view, params, mContext.display, mParentWindow)
}
```
...
fun makeVisible() {
    if (!mWindowAdded) {
        ...
        wm.addView(mDecor, window.attributes)
        mWindowAdded = true
    }
...
```kotlin
// Dialog.java
Dialog(context: Context, themeResId: Int, createContextThemeWrapper: Boolean) {
    ...
    mWindowManager = context.getSystemService(Context.WINDOW_SERVICE) as WindowManager
    val w = PhoneWindow(mContext)
    mWindow = w
    w.setCallback(this)
    w.setWindowManager(mWindowManager, null, null)
